...
import { Wallet, Mail, ArrowRight, ArrowLeft, CheckCircle2, AlertCircle } from 'lucide-react';
...
export const ForgotPassword: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message || 'Request failed. Please check your email.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-950 text-slate-100 p-4 sm:p-6 relative overflow-hidden">
      ...
      <div className="absolute top-1/3 -left-20 w-72 h-72 bg-violet-600/30 rounded-full blur-[100px] pointer-events-none" />
      <div className="absolute bottom-1/3 -right-20 w-80 h-80 bg-indigo-600/25 rounded-full blur-[120px] pointer-events-none" />
        ...
        <div className="bg-slate-900/80 backdrop-blur-xl border border-slate-800/80 rounded-3xl p-6 sm:p-8 shadow-2xl">
          ...
          {successMessage ? (
            <div className="space-y-6 text-center animate-fadeIn">
              ...
              <div>
                <h2 className="text-lg font-bold text-white mb-2">Check Your Request</h2>
              ...
              {resetToken && (
                <div className="p-4 rounded-2xl bg-slate-950/80 border border-slate-800 text-left space-y-2">
                  <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Demo Password Reset Token:</p>
                  <p className="text-xs font-mono text-indigo-400 break-all select-all bg-slate-900 p-2 rounded-lg border border-slate-800">{resetToken}</p>
                  <button
                    onClick={() => navigate(`/auth/reset-password?token=${resetToken}`)}
                    className="w-full mt-2 py-2 px-4 rounded-xl bg-indigo-600/30 hover:bg-indigo-600/40 text-indigo-300 font-medium text-xs flex items-center justify-center gap-2 border border-indigo-500/30 transition-colors"
                  >
              ...
              <Link
                to="/auth/login"
                className="inline-flex items-center justify-center gap-2 text-sm font-semibold text-indigo-400 hover:text-indigo-300 transition-colors pt-2"
              >
